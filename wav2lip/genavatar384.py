...
parser = argparse.ArgumentParser(description='Generate 384x384 avatar for Wav2Lip384 model')
parser.add_argument('--img_size', default=384, type=int,
                    help='Face image size (default: 384 for wav2lip384)')
parser.add_argument('--avatar_id', default='wav2lip384_avatar1', type=str,
                    help='Avatar identifier')
...
if __name__ == "__main__":
    # Output to project root data/avatars directory
    script_dir = os.path.dirname(os.path.abspath(__file__))
    project_root = os.path.dirname(script_dir)
    avatar_path = os.path.join(project_root, "data", "avatars", args.avatar_id)
    full_imgs_path = f"{avatar_path}/full_imgs"
    face_imgs_path = f"{avatar_path}/face_imgs"
    coords_path = f"{avatar_path}/coords.pkl"
    osmakedirs([avatar_path, full_imgs_path, face_imgs_path])
    print(args)
    print(f"Generating 384x384 avatar to: {avatar_path}")
    ...
    print(f"  - Face images: {idx} frames at {args.img_size}x{args.img_size}")
    print(f"  - Output path: {avatar_path}")
