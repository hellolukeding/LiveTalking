#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
DoubaoTTS优化效果测试脚本

测试目标：
1. 验证长文本消息不会丢失
2. 验证唇形驱动正常工作
3. 验证音频流完整传输
"""

from logger import logger
from fixes.optimize_doubao_playback import DoubaoPlaybackOptimizer
import numpy as np
from unittest.mock import MagicMock, Mock, patch
import time
import threading
import queue
import asyncio
import os
import sys

sys.path.insert(0, '/Users/lukeding/Desktop/playground/2025/LiveTalking')


class MockTTS:
    """模拟TTS实例"""

    def __init__(self):
        self.state = Mock()
        self.state.RUNNING = 0
        self.state.state = 0

        self.chunk = 320
        self.audio_track = None
        self.loop = None
        self.parent = Mock()

        # 模拟方法
        self._original_txt_to_audio = Mock()
        self._original_stream_audio = Mock()

    def put_audio_frame(self, audio_chunk, eventpoint):
        """模拟音频帧处理"""
        logger.info(
            f"[MOCK] put_audio_frame: {len(audio_chunk)} samples, event={eventpoint}")

    def txt_to_audio(self, msg):
        """原始方法"""
        logger.info(f"[MOCK] 原始txt_to_audio: {msg[0][:50]}...")

    def stream_audio(self, audio_array, msg):
        """原始方法"""
        logger.info(f"[MOCK] 原始stream_audio: {len(audio_array)} samples")


class MockLipASR:
    """模拟唇形驱动"""

    def __init__(self):
        self.feat_queue = queue.Queue()
        self.output_queue = queue.Queue()
        self.audio_frames_received = 0

    def put_audio_frame(self, audio_chunk, eventpoint):
        """接收音频帧"""
        self.audio_frames_received += 1
        logger.info(
            f"[MOCK LipASR] 收到音频帧 #{self.audio_frames_received}: {len(audio_chunk)} samples")

        # 模拟特征提取和输出
        self.feat_queue.put("mock_features")

        # 模拟唇形帧输出
        mock_frame = np.random.randint(0, 255, (256, 256, 3), dtype=np.uint8)
        self.output_queue.put((mock_frame, 0, [(audio_chunk, 0, eventpoint)]))


class MockAudioTrack:
    """模拟WebRTC音频轨道"""

    def __init__(self):
        self._queue = queue.Queue(maxsize=100)
        self.frames_received = 0

    def put_frame(self, frame, eventpoint):
        """接收音频帧"""
        self.frames_received += 1
        try:
            self._queue.put_nowait((frame, eventpoint))
            logger.info(f"[MOCK AudioTrack] 收到音频帧 #{self.frames_received}")
        except queue.Full:
            logger.warning("[MOCK AudioTrack] 队列满，丢弃帧")


class MockLoop:
    """模拟事件循环"""

    def call_soon_threadsafe(self, callback, *args):
        """线程安全调用"""
        try:
            callback(*args)
        except Exception as e:
            logger.error(f"[MOCK Loop] 调用失败: {e}")


def test_optimization():
    """测试优化效果"""
    print("=" * 70)
    print("🚀 DoubaoTTS 优化效果测试")
    print("=" * 70)

    # 1. 创建模拟组件
    print("\n1️⃣ 创建模拟组件...")
    mock_tts = MockTTS()
    mock_lip_asr = MockLipASR()
    mock_audio_track = MockAudioTrack()
    mock_loop = MockLoop()

    # 设置TTS组件
    mock_tts.audio_track = mock_audio_track
    mock_tts.loop = mock_loop
    mock_tts.parent.lip_asr = mock_lip_asr

    print("   ✅ 模拟组件创建完成")

    # 2. 应用优化器
    print("\n2️⃣ 应用优化器...")
    optimizer = DoubaoPlaybackOptimizer(mock_tts, mock_lip_asr)

    # 设置直接转发
    optimizer.setup_direct_forwarding()
    print(f"   ✅ LipASR就绪: {optimizer.lip_asr_ready}")

    # 修补方法
    optimizer.patch_tts_methods()
    print("   ✅ 方法修补完成")

    # 3. 测试长文本处理
    print("\n3️⃣ 测试长文本处理...")
    long_text = "这是一个非常长的测试文本，包含多个句子。" * 10  # 100+字符
    logger.info(f"测试文本长度: {len(long_text)}字符")

    # 模拟音频数据
    audio_data = np.random.randn(16000 * 5).astype(np.float32)  # 5秒音频

    # 调用优化后的stream_audio
    msg = (long_text, {'test': True})
    mock_tts.stream_audio(audio_data, msg)

    print(f"   ✅ 长文本处理完成")

    # 4. 验证结果
    print("\n4️⃣ 验证结果...")

    # 检查音频轨道接收
    audio_frames = mock_audio_track.frames_received
    print(f"   📊 WebRTC音频帧: {audio_frames}")

    # 检查唇形驱动接收
    lip_frames = mock_lip_asr.audio_frames_received
    print(f"   👄 唇形驱动帧: {lip_frames}")

    # 检查队列状态
    webrtc_queue_size = mock_audio_track._queue.qsize()
    lip_output_size = mock_lip_asr.output_queue.qsize()

    print(f"   📦 WebRTC队列: {webrtc_queue_size}")
    print(f"   📦 LipASR输出队列: {lip_output_size}")

    # 5. 性能评估
    print("\n5️⃣ 性能评估...")

    success = True
    if audio_frames == 0:
        print("   ❌ 失败: 没有音频帧发送到WebRTC")
        success = False
    else:
        print("   ✅ 音频帧正常发送")

    if lip_frames == 0:
        print("   ❌ 失败: 没有音频帧转发到唇形驱动")
        success = False
    else:
        print("   ✅ 唇形驱动正常工作")

    if audio_frames > 0 and lip_frames > 0:
        print("   🎉 优化成功! 消息丢失和唇形驱动失效问题已解决")
    else:
        print("   ⚠️  部分功能未正常工作")

    # 6. 状态报告
    print("\n6️⃣ 状态报告...")
    status = optimizer.get_status_report()
    print(f"   LipASR就绪: {status['lip_asr_ready']}")
    print(f"   音频轨道就绪: {status['audio_track_ready']}")
    print(f"   总发送帧数: {status['audio_stats']['total_sent']}")
    print(f"   唇形驱动帧数: {status['audio_stats']['lip_driven_frames']}")

    print("\n" + "=" * 70)
    if success:
        print("✅ 测试通过! DoubaoTTS优化已成功应用")
    else:
        print("❌ 测试失败，请检查优化实现")
    print("=" * 70)

    return success


def test_message_queue():
    """测试消息队列处理"""
    print("\n" + "=" * 70)
    print("📨 消息队列测试")
    print("=" * 70)

    from fixes.optimize_doubao_playback import DoubaoPlaybackOptimizer

    mock_tts = MockTTS()
    optimizer = DoubaoPlaybackOptimizer(mock_tts, None)

    # 测试文本分割
    test_cases = [
        ("短文本", 1),
        ("这是一个中等长度的文本，应该不会被分割", 1),
        ("这是一个非常长的文本，包含多个句子。" * 5, 3),  # 长文本，应该分割
    ]

    for text, expected_chunks in test_cases:
        print(f"\n测试: {text[:30]}... (长度: {len(text)})")

        # 模拟优化后的txt_to_audio
        chunks = []
        max_length = 200

        if len(text) > max_length:
            import re
            segments = re.split(r'([。！？；,.!?;])', text)
            segments = [s.strip() for s in segments if s.strip()]

            current_chunk = ""
            for seg in segments:
                if len(current_chunk) + len(seg) < max_length:
                    current_chunk += seg
                else:
                    if current_chunk:
                        chunks.append(current_chunk)
                    current_chunk = seg
            if current_chunk:
                chunks.append(current_chunk)
        else:
            chunks = [text]

        print(f"   分割结果: {len(chunks)}块")
        print(f"   预期: {expected_chunks}块")

        if len(chunks) <= expected_chunks:
            print("   ✅ 分割合理")
        else:
            print("   ⚠️  分割可能过多")


if __name__ == "__main__":
    # 运行测试
    success = test_optimization()
    test_message_queue()

    # 退出码
    sys.exit(0 if success else 1)
