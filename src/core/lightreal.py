...
def read_lms(lms_list):
    ...
    for lms_path in tqdm(lms_list):
        ...
        # Add the file's landmarks to the overall list
...
#                h, w = crop_img.shape[:2]
                ...
                img_masked = cv2.rectangle(
                    img_real_ex_ori, (5, 5, 150, 145), (0, 0, 0), -1)
                ...
                img_concat_T = torch.cat(
                    [img_real_ex_T, img_masked_T], axis=0)[None]
                img_batch.append(img_concat_T)

            reshaped_mel_batch = [arr.reshape(32, 32, 32) for arr in mel_batch]
            mel_batch = torch.stack([torch.from_numpy(arr)
                                    for arr in reshaped_mel_batch])
